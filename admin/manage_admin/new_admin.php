...
<?php
    ...
    $nic = $password = $first_name=$last_name=$confirm_password=$hosid= "";
    $nic_err = $password_err = $first_name_err = $last_name_err = $confirm_password_err = $hosid_err = "";
    ...
    if(empty(trim($_POST["hosid"]))){
         $hosid_err = "Please enter an Hospital ID.";
    }else{
        // Prepare a select statement 
        ...
        if($stmt2 = mysqli_prepare($link, $sql2)){
            ...
            if(mysqli_stmt_execute($stmt2)){
                ...
                if(mysqli_stmt_num_rows($stmt2) == 0){
                    $hosid_err = "There is no such kind of Hospital ID.";
                } else{
    ...
    if(empty($nic_err) && empty($password_err) && empty($confirm_password_err) && empty($first_name_err) && empty($last_name_err) && empty($hosid_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO blood_bank_admin (NIC, Password, FirstName, LastName, BloodBankID) VALUES (?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssss", $param_username, $param_password, $first_name, $last_name, $hosid);
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                header("location: manage_admin.php?reg=ok");
            } else{
