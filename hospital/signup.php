<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST")
    {
        ...
        {
            //prepare statement
            $sql="SELECT NHospitalID FROM normal_hospital WHERE UserName=?";
        ...
        if(empty($username_err)&& empty($password_err) && empty($confirm_password_err) && empty($mobile_err) && empty($dr_err) && empty($district_err) && empty($address_err) && empty($hos_err))
        {
            //prepare insert statemet
            $sql= "INSERT INTO normal_hospital (Name, Address, District, Chief, UserName,Password) VALUES(?, ?, ?, ?, ?, ?)";

            if($stmt=mysqli_prepare($link,$sql))
            {
                mysqli_stmt_bind_param($stmt,"ssssss",$hos_name,$address,$district,$chief_doctor,$param_username,$param_password);
			...
			if(empty($mobile_err))
            {
                ...
                {
                    $sql="INSERT INTO normal_hospital_telephone (TelephoneNo,NhId) VALUES(?,?)";
                    if($stmt=mysqli_prepare($link,$sql))
                    {
                        mysqli_stmt_bind_param($stmt,"ii",$param_mobile,$param_id);
                        $param_mobile=$mobile;
                        $param_id=$data;
                    ...
                    if($stmt=mysqli_prepare($link,$sql))
                    {
                        ...
                        $param_id=$data;
                        mysqli_stmt_execute($stmt);
                        mysqli_stmt_bind_param($stmt,"ii",$param_mobile2,$param_id2);
                        $param_mobile2=trim($_POST["mobile2"]);
                        $param_id2=$data;
                        mysqli_stmt_execute($stmt);
                        header("location: ../reg_login.php");
