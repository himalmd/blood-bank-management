...
// Initialization
$old_err=$new_err=$confirm_err=$server_err="";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(empty(trim($_POST["old"])))
        {
            ...
            $password= $_POST["old"];
            $sql = "SELECT Password FROM requestor WHERE NIC= '$nic'";
        ...
        if (empty($old_err) && empty($new_err) && empty($confirm_err) && empty($server_err)) {
        	...
        	$sql2= "UPDATE requestor SET Password='$param_password' WHERE NIC='$nic'";
        	if (mysqli_query($link, $sql2)) {
        		// Redirect to update page
                header("location: edit-requester?password=ok");
	...
	<div class="main clearfix">
		...
		<center>
			<form action="" method="post">
                ...
                <a href="edit-requester" style="color: #848484; font-size: 15px;">Cancel</a>
            </div>
			</form>
		</center>
		</div>
