...
$nic = $password = $confirm_password = $first_name = $last_name = $dob = $gender = $addline1 = $addline2 = $telephone = $district = "";
$nic_err = $password_err = $confirm_password_err = $first_name_err = $last_name_err = $dob_err = $gender_err = $addline1_err = $addline2_err = $telephone_err = $district_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($nic_err) && empty($password_err) && empty($confirm_password_err) && empty($dob_err) && empty($gender_err) && empty($telephone_err) && empty($district_err) && empty($first_name_err) && empty($last_name_err) && empty($addline1_err)){
        ...
        $sql = "INSERT INTO requestor (NIC, Password, FirstName, LastName, DateOfBirth, Gender, District, Lane1, Lane2) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssssss", $param_username, $param_password, $first_name, $last_name, $dob, $gender, $district, $addline1, $addline2);
