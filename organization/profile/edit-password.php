...
$old_err=$new_err=$confirm_err="";
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if(empty(trim($_POST["old"])))
        {
            ...
            $password= $_POST["old"];
            $sql = "SELECT Password FROM organization WHERE UserName= '$nic'";
        ...
        if (empty($old_err) && empty($new_err) && empty($confirm_err) && empty($server_err)) {
        	...
        	$sql2= "UPDATE organization SET Password='$param_password' WHERE UserName='$nic'";
        	if (mysqli_query($link, $sql2)) {
        		// Redirect to update page
                header("location: edit-organization?password=ok");
	...
	<div class="main clearfix">
		...
		<center>
			<form action="" method="post">
                ...
                <a href="edit-organization" style="color: #848484; font-size: 15px;">Cancel</a>
            </div>
			</form>
		</center>
		</div>
